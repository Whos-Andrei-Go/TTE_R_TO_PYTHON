...
class TeWeightsSpec:
    ...
    def __str__(self):
        """Returns a readable summary of the weight model."""
        output = []
        output.append(f"  - Numerator formula: {self.numerator}")
        output.append(f"  - Denominator formula: {self.denominator}")
        if self.pool_numerator and self.pool_denominator:
            output.append("  - Both numerator and denominator models are pooled.")
        elif self.pool_numerator:
            output.append("  - Numerator model is pooled, denominator is not.")
        elif self.pool_denominator:
            output.append("  - Denominator model is pooled, numerator is not.")
        output.append(f"  - Model fitter: {type(self.model_fitter).__name__}")
        output.append("  - Fitted models: " + (", ".join(self.fitted.keys()) if self.fitted else "Not fitted yet."))
...
class TeWeightsUnset(TeWeightsSpec):
    ...
    def __str__(self):
        """Returns a readable summary for unset weights."""
        return "  - No weight model specified."
